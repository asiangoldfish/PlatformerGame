...
//  ImGuiPlatformIO interface. 2023-06-20: OpenGL: Fixed erroneous use
...
#define GL_CALL(_CALL)                                                         \
    do {                                                                       \
        ...
        if (gl_err != 0)                                                       \
            fprintf(                                                           \
              stderr, "GL error 0x%x returned from '%s'.\n", gl_err, #_CALL);  \
...
static void
ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data,
                                   int fb_width,
                                   int fb_height,
...
static void
ImGui_ImplOpenGL3_RenderWindow(ImGuiViewport* viewport, void*)
{
