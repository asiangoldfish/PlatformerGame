...
SokobanApplication::init()
{
    ...
    // ------------
    map = new Map(shader);
    map->addNewMap("test", RESOURCES_DIR + std::string("/maps/test.map"));
    map->loadMap("test");
    ...
    // Floor
    floor = new Floor(shader, glm::vec2(mapSize), glm::vec2(tileSize));
    floor->setTextureName("floor");
    ...
    for (int i = 0; i < maxPillars; i++) {
        Cube* newPillar = new Cube(gApp->getShader());
    ...
    for (int i = 0; i < maxDestinations; i++) {
        Cube* newDestination = new Cube(gApp->getShader());

        newDestination->setScale({ 0.8f, 0.1f, 0.8f });
        newDestination->move(
          { 0.0f, newDestination->getScale().y / 2.0f, 0.0f });
    ...
    for (int i = 0; i < maxBoxes; i++) {
        Cube* newBox = new Cube(gApp->getShader());
    ...
    camera = std::make_shared<Framework::PerspectiveCamera>(
      Framework::PerspectiveCamera::Frustrum(
        40.0f, 1024.0f, 1024.0f, 0.01f, 400.0f),
      glm::vec3{ worldCenter + glm::cos(glm::radians(degrees)) * cameraDistance,
                 10.0f,
                 worldCenter +
                   glm::sin(glm::radians(degrees)) * cameraDistance }
...
SokobanApplication::run()
{
    while (!glfwWindowShouldClose(getWindow())) {
        ...
        // Wall
        Framework::TextureManager::bind("wall");
        for (const auto& wall : walls) {
            // wall->draw();
        }

        // Pillars
        Framework::TextureManager::bind("pillar");
        for (const auto& pillar : pillars) {
            // pillar->draw();
        }

        // Destinations
        Framework::TextureManager::bind("destination");
        for (const auto& destination : destinations) {
            // destination->draw();
        }

        // Boxes
        Framework::TextureManager::bind("box");
