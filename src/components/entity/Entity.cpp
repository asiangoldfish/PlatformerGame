...
void
Entity::initDrawable(Framework::Shader* shader,
                     ...
                     std::vector<uint32_t> indices)
{
    ...
    indexBuffer = new Framework::IndexBuffer(indices.data(), indices.size());
...
void
Entity::rotate(float yawBy, float pitchBy, float rollBy)
{
    yaw += yawBy;
    pitch += pitchBy;
    roll += rollBy;
...
void
Entity::setRotation(float yaw, float pitch, float roll)
{
    this->yaw = yaw;
    this->pitch = pitch;
    this->roll = roll;
...
void
Entity::setColor(const glm::vec4& color)
{
    this->color = color;
}
