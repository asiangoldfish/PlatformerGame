...
void Emitter::addParticle(int amount) {
    ...
    for (int i = 0; i < amount && numOfParticles < maxParticles; i++) {
        ...
        particle->color =
          glm::vec4(rng(0.0f, 1.0f), rng(0.0f, 1.0f), rng(0.0f, 1.0f), 1.0f);
...
void Emitter::recalculateModelMatrix(glm::vec3 translate,
    ...
    // Rotation
    rotx =
      glm::rotate(modelMatrix, glm::radians(rotate.x), { 1.0f, 0.0f, 0.0f });
    roty =
      glm::rotate(modelMatrix, glm::radians(rotate.y), { 0.0f, 1.0f, 0.0f });
    rotz =
      glm::rotate(modelMatrix, glm::radians(rotate.z), { 0.0f, 0.0f, 1.0f });
...
ParticleShape::ParticleShape() {
    ...
    auto vertices = UnitGridGeometry2D(VERTEX_ATTRIBUTE::POSITION);
