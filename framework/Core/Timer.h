 ...
 * Measure time.
 *
 * @details Track time elapsed from certain place in code to another. The
 * class can be instanced, so multiple timers can be used to track time.
 *
 * @details By default, time is measured in milliseconds. When using
 * <u>update()</u>, make sure to pass in number of millimeters to increment
 * time by.
 *
...
public:
     ...
     *
     * @details Call this function on every frame to get the most accurate
     * result.
     *
     * @param milliseconds Milliseconds to increment <u>elapsedTime</u> by.
     */
    inline void update(const float milliseconds) {
     ...
     *
     * @details This function is ideally called at the beginning of every
     * frame.
     */
