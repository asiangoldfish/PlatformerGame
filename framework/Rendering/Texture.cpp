...
uint32_t Texture::loadTexture2D(const std::string& name_,
    ...
    glTextureSubImage2D(
      textureID, 0, 0, 0, width, height, dataFormat, GL_UNSIGNED_BYTE, pixels);
...
uint32_t Texture::loadCubeMap(const std::string& name_,
                              const std::string& filepath_) {
    std::vector<std::string> filePaths = { filepath_, filepath_, filepath_,
                                           filepath_, filepath_, filepath_ };
