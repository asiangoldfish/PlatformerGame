...
GLuint Shader::compileShader(GLenum shaderType, const std::string& shaderSrc) {
    ...
    if (!success) {
        ...
        FATAL("ERROR::SHADER::{0}::COMPILATION_FAILED\n{1}", typeName, infoLog);
...
void Shader::setFloat2(const std::string& name, const glm::vec2& vector) const {
    int location = glGetUniformLocation(shaderProgram, name.c_str());
    OUTPUT_SHADER_LOCATION_LOG(location, name);
    glUniform2f(location, vector.x, vector.y);
}

void Shader::setFloat3(const std::string& name, const glm::vec3& vector) const {
    int location = glGetUniformLocation(shaderProgram, name.c_str());
    OUTPUT_SHADER_LOCATION_LOG(location, name);
    glUniform3f(location, vector.x, vector.y, vector.z);
}

void Shader::setFloat4(const std::string& name, const glm::vec4& vector) const {
    int location = glGetUniformLocation(shaderProgram, name.c_str());
    OUTPUT_SHADER_LOCATION_LOG(location, name);
